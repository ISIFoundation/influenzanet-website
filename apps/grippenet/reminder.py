...
def get_login_url(user, next_uri):
    expires = datetime.datetime.now() + datetime.timedelta(days=30)

    usage_left = 5
    key = loginurl.utils.create(user, usage_left, expires, next_uri)
...
def create_reminder_message(user, next=None, template_name='reminder', wrap_layout=True, data=None):
    ...
    data['url'] = get_login_url(user, next)

    data.update(site_context())
    ...
    return create_message_from_template(template_name, data, wrap_layout=wrap_layout)