...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'Ranking.User'
        db.delete_column('RankingService', 'User_id')

        # Adding field 'Ranking.user'
        db.add_column('RankingService', 'user', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'], null=True), keep_default=False)

        # Adding unique constraint on 'Ranking', fields ['service_id', 'user']
        db.create_unique('RankingService', ['service_id_id', 'user_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Ranking', fields ['service_id', 'user']
        db.delete_unique('RankingService', ['service_id_id', 'user_id'])

        # User chose to not deal with backwards NULL issues for 'Ranking.User'
        raise RuntimeError("Cannot reverse this migration. 'Ranking.User' and its values cannot be restored.")
